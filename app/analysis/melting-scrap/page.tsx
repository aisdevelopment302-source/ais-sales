...
export default function MeltingScrapAnalysisPage() {
    ...
    try {
      ...
      salesSnap.forEach((doc) => {
        ...
        saleItems.forEach((si) => {
          if (Number(si.itemcode) !== MELTING_SCRAP_ID) return;
  ...
  const kpis: SummaryKPI[] = [
    { label: "Total Sales (₹)", value: formatCurrency(totalSales), color: "#8b5cf6" },
    { label: "Total Weight (kg)", value: formatNumber(Math.round(totalWeight)), color: "#a78bfa" },
    { label: "Avg Rate/kg", value: avgRate.toFixed(2), color: "#c4b5fd" },
    { label: "Avg Monthly Sales", value: formatCurrency(avgMonthly), color: "#7c3aed" },
  ];
          ...
          <div className="section-card">
            ...
            <ResponsiveContainer width="100%" height={400}>
              <AreaChart data={monthly}>
                ...
                <Legend />
                <Area type="monotone" dataKey="total_sales" fill="#c4b5fd" stroke="#8b5cf6" name="Sales (₹)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
